        ...
        <div v-if="ID!==undefined&&ID!==null">
          <div>
            <v-card-title primary-title>
              <h3 class="headline mb-0">Editando disciplina</h3>
            </v-card-title>
          </div>
        </div>
        <div v-else>
          <div>
            <v-card-title primary-title>
              <h3 class="headline mb-0">Cadastro de disciplina</h3>
            </v-card-title>
          </div>
        </div>
        <v-flex>
          <v-form ref="form">
            ...
            <div v-if="ID!==undefined&&ID!==null">
            <v-btn color="success" @click="EditarDisciplina">
              Editar
            </v-btn>
            </div>
            <div v-else>
            <v-btn color="success" @click="SalvarDisciplina">
              Cadastrar
            </v-btn>
            </div>
          </v-form>
...
export default {
  ...
  methods: {
    SalvarDisciplina: function() {
      ...
      ServiceDisciplina.CriarDisciplina(obj);
    },
    EditarDisciplina: function() {
      var obj = {
        ID: this.ID,
        Nome: this.Nome,
        Nascimento: new Date(this.Nascimento).toISOString(),
      ...
      ServiceDisciplina.EditarDisciplina(obj);
    }
  }
};
</script>